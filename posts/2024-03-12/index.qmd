...
### I want to make plots in an open source way, how?

This blog post won't give a full tutorial, just check out a YouTube video for that, there's really loads! Or even ask chatgpt to do it for you, since it seems to do well at basic coding anyway. 

With a fully configured neovim setup (like the one I have) it makes coding easier and much faster to do. This is an additional reason to move away from MATLAB since you almost have to do everything inside MATLABs gui. 

### Bar plots

Here's an example bar plot in R using `ggplot`:
...
Here are some examples of scatter plots using python:
...
This is all there is to it. So the main trick here is to get the data in a format that can be read correctly. The sure fire way is to make sure the data provided to the plots is as a type array. But of course it's possible to index for example a pandas dataframe as an array. 
