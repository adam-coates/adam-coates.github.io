...
1. First the script converts the image into a temporary text file. The text file is essentially coordinates of the pixel plus what the pixel represents in RGB, hex and sRGB format. So a typical row might look like: 
...
- Here the user can specify what colour to search for by using the following flag `-c` plus the colour in RGB format without spaces i.e. `-c 255,0,0`

2. The script uses this in order to find the pixels with a particular colour and saves the coordinates. Coordinates are first split into x and y by a comma

Here's an example of the coordinates:
...
The `head` and `tail` commands are used to get the lowest coordinate and the highest coordinate. These coordinates are then the 'bounding box' in which we want to cut out or draw a box around. 

Here's the maximum and minimum x and y coordinates with the padding: 
...
3. The easiest way for imagemagick to cut out a rectangle from an image is to create a mask first. So here we create a mask of the first image that was used to get the coordinates from. The mask is a black and white image that shows region to cut out. The second command uses the mask to cut out the original image.
...
![The image is now cropped](./crop.png){fig-align='center' width=75%}


</center>


4. I added another option in the script since sometimes we only want to draw an outline of the region on the original image this outputs the following image: 
...
- In the future I will perhaps add options when running the command in order to change how the output will look like. 
