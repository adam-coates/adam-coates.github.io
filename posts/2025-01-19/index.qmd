---
title: "Finding the optimal note-taking solution"
description: "My 2 year long story behind trying to find the most optimal note-taking system"
image: "obsidian_graph_view.png"
comments:
  giscus:
    repo: "adam-coates/adam-coates.github.io"
    mapping: "title"
date: "2025-01-19"
categories: ["note-taking", "neovim", "Obsidian"]
draft: false
...
When I first heard of [Obsidian](https://obsidian.md/), back around 2021, I remember I had tried it out for about an hour and decided that I didn't like it. I couldn't get used to the UI, and I struggled to find a way that would allow me to access my notes no matter which computer I was using. Long story short, I went straight back to my usual and at the time comforting handwritten notebooks. It wasn't until I discovered [neovim](https://neovim.io/) and made my very first configuration that I started to get into digital note-taking (here is a link to my first neovim configuration commit, <a href="https://github.com/adam-coates/dotfiles/commits/main/?since=2023-10-04&until=2023-10-04" class="button">97ca162</a>).

I became interested in a neovim plugin called [Vimwiki](https://github.com/vimwiki/vimwiki). Put simply, vimwiki is a wiki-like note-taking system that can be used in neovim. 

<center>

<span style="font-size: 1.25em; font-style: italic;">Had I found the solution to my note-taking problem?</span>
...
There is actually nothing too much wrong with Vimwiki. It was a great first tool that got me into digital note-taking. I quite liked the fact that a single plugin would allow me to make notes efficiently, all the while a plugin like [telescope](https://github.com/nvim-telescope/telescope.nvim) enabled me to do keyword searching for notes. This meant that I had solved at least one problem: information retrieval. 
...
<a href="https://github.com/adam-coates/dotfiles/blob/97ca1621f9044cdbc5188c989a1847f0e0112e6a/.config/nvim/lua/adam/plugins/Vimwiki.lua" class="button">vimwiki.lua</a>
...
1. Syncing
2. Editing my markdown files on a device without neovim (particularly on a smartphone)
...
1. Google Keep technically had its own syntax and was also not Markdown compliant
2. Being locked into Google's architecture 
3. No way of linking notes together
4. No official Google Keep API
5. No official in-app way to convert Google Keep notes to other files
...
To solve point five, I found a python script called [keep-it-markdown](https://github.com/djsudduth/keep-it-markdown). This essentially meant that I could convert any and all Google Keep notes to Markdown. The script was relatively easy to use and meant that I could convert my notes and back them up to a cloud provider. However, because there is no official API in Google Keep the setup of this was quite involved and technically quite challenging. Still, this solved point five and kind of point one. 

Since wanting to stick to editing notes inside neovim because of the many benefits described [in my other blog post](https://adam-coates.github.io/posts/2024-03-08/) I came across a plugin called [gkeep](https://github.com/stevearc/gkeep.nvim). 

&nbsp;

#### Google Keep's neovim plugin

This plugin was again, at the time, I thought **revolutionary**. Here was my config and commit where I first started using it <a href="https://github.com/adam-coates/dotfiles/commits/main/?since=2023-12-13&until=2023-12-14" class="button">09d3e78</a> : 
...
<span style="font-size: 1.25em; font-style: italic;">"I need folders!"</span>
...
" You don’t really know how to do something<br>
...
This is where using Git has been super handy for me. I use [Lazygit](https://github.com/jesseduffield/lazygit) with [Lazygit.nvim](https://github.com/kdheepak/lazygit.nvim) as a neovim plugin that basically means I can set a keymap to open Lazygit and run `git add .` `git commit -m` and `git push` all within a few seconds. 
...
I can use the Obsidian mobile app for editing markdown files on the fly {{< fa solid rocket >}} 
...
It seems as though my search for note-taking system that actually works has gone full circle (like the Obsidian graph view {{< fa solid face-smile-wink >}})

![My current Obsidian graph view](./obsidian_graph_view.png){width=50%}


I started out by not liking Obsidian but by the end have really fallen in love with it {{< fa solid heart >}}. Combine Obsidian, neovim and git you end up with quite a lovely streamlined note-taking system.
